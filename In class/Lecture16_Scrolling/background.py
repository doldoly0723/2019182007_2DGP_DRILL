...
class FixedBackground:
    ...
    def update(self):
        self.window_left = clamp(0,
                    int(server.boy.x) - self.canvas_width // 2, self.w - self.canvas_width - 1)

        self.window_bottom = clamp(0,
                    int(server.boy.y) - self.canvas_height // 2, self.h - self.canvas_height - 1)
...
class TileBackground:
    ...
    def draw(self):
        ...
        title_left = self.window_left // 800
        title_right = (self.window_left + self.canvas_width) // 800
        left_offset = self.window_left % 800

        title_bottom = self.window_bottom // 600
        title_top = (self.window_bottom + self.canvas_height) // 600
